...
static int lldi_ind_dir_reg(vm_t *vm, prog_t *prog)
{
    short int ind = 0;
    short int sum[2] = {0};
    int reg = 0;
...
static int lldi_ind_reg_reg(vm_t *vm, prog_t *prog)
{
    int reg[2] = {0};
    short int ind = 0;
    short int sum[2] = {0};
...
{
    if (arg[1] == T_DIR)
        return lldi_ind_dir_reg(vm, prog);
    if (arg[1] == T_REG)
        return lldi_ind_reg_reg(vm, prog);
    modify_carry(prog, false);
    return 84;
}
