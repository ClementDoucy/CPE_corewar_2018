...
static int ld_dir_reg(vm_t *vm, prog_t *prog)
{
    unsigned int dir1 = 0;
    int reg2 = 0;
...
static int ld_ind_reg(vm_t *vm, prog_t *prog)
{
    short int ind1 = 0;
    int reg2 = 0;
    int value_to_load = 0;
    ...
    my_memcpy(&value_to_load, \
        &vm->mem[(prog->pc + ind1 % IDX_MOD) % MEM_SIZE], REG_SIZE);
...
int ld(vm_t *vm, prog_t *prog)
{
    ...
    if (arg[0] == T_DIR)
        return ld_dir_reg(vm, prog);
    else if (arg[0] == T_IND)
        return ld_ind_reg(vm, prog);
    modify_carry(prog, false);
    return 84;
}
