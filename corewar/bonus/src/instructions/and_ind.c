...
static int and_ind_reg_reg(vm_t *vm, prog_t *prog)
{
    int reg[2] = {0};
    int value_to_and = 0;
    short int index = 0;
...
static int and_ind_dir_reg(vm_t *vm, prog_t *prog)
{
    int reg = 0;
    int value_to_and[2] = {0};
    short int index = 0;

    my_memcpy(&value_to_and[0], &vm->mem[(prog->pc + 4) % MEM_SIZE], DIR_SIZE);
...
static int and_ind_ind_reg(vm_t *vm, prog_t *prog)
{
    int reg = 0;
    short int index[2] = {0};
    int v_and[2] = {0};
    my_memcpy(&reg, &vm->mem[prog->pc + 6], 1);
    if (!is_reg_exist(reg)) {
        modify_carry(prog, false);
        return 84;
    }
    ...
    my_memcpy(&v_and[0], &vm->mem[(prog->pc + index[0]) % MEM_SIZE], DIR_SIZE);
    my_memcpy(&v_and[1], &vm->mem[(prog->pc + index[1]) % MEM_SIZE], DIR_SIZE);
    v_and[0] = convert_endian_number(v_and[0]);
    v_and[1] = convert_endian_number(v_and[1]);
    prog->r[reg - 1] = v_and[0] & v_and[1];
...
{
    if (arg[1] == T_REG)
        return and_ind_reg_reg(vm, prog);
    if (arg[1] == T_DIR)
        return and_ind_dir_reg(vm, prog);
    if (arg[1] == T_IND)
        return and_ind_ind_reg(vm, prog);
    modify_carry(prog, false);
    return 84;
}
