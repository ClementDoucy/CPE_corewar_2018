...
static int lldi_reg_dir_reg(vm_t *vm, prog_t *prog)
{
    int reg[2] = {0};
    short int sum[2] = {0};
...
static int lldi_reg_reg_reg(vm_t *vm, prog_t *prog)
{
    int reg[3] = {0};
    short int sum[2] = {0};
    ...
    my_memcpy(&reg[2], &vm->mem[(prog->pc + 4) % MEM_SIZE], 1);
    if (!is_reg_exist(reg[0]) || !is_reg_exist(reg[1]) || \
        !is_reg_exist(reg[2])) {
        modify_carry(prog, false);
        return 84;
    }
    my_memcpy(&sum[0], &prog->r[reg[0] - 1], IND_SIZE);
    my_memcpy(&sum[1], &prog->r[reg[1] - 1], IND_SIZE);
    my_memcpy(&prog->r[reg[2] - 1], &vm->mem[(prog->pc + \
                ((int)sum[0] + (int)sum[1])) % MEM_SIZE], REG_SIZE);
    prog->r[reg[2] - 1] = convert_endian_number(prog->r[reg[2] - 1]);
...
{
    if (arg[1] == T_DIR)
        return lldi_reg_dir_reg(vm, prog);
    if (arg[1] == T_REG)
        return lldi_reg_reg_reg(vm, prog);
    modify_carry(prog, false);
    return 84;
}
