...
static int xor_reg_reg_reg(vm_t *vm, prog_t *prog)
{
    int reg[3] = {0};
...
static int xor_reg_dir_reg(vm_t *vm, prog_t *prog)
{
    int reg[2] = {0};
    int value_to_xor = 0;
    ...
    my_memcpy(&value_to_xor, &vm->mem[(prog->pc + 3) % MEM_SIZE], DIR_SIZE);
...
static int xor_reg_ind_reg(vm_t *vm, prog_t *prog)
{
    int reg[2] = {0};
    short int index = 0;
    int value_to_xor = 0;
...
{
    if (arg[1] == T_REG)
        return xor_reg_reg_reg(vm, prog);
    if (arg[1] == T_DIR)
        return xor_reg_dir_reg(vm, prog);
    if (arg[1] == T_IND)
        return xor_reg_ind_reg(vm, prog);
    modify_carry(prog, false);
    return 84;
}
