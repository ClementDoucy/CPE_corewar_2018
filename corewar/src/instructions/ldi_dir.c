...
static int ldi_dir_dir_reg(vm_t *vm, prog_t *prog)
{
    short int sum[2] = {0};
    int reg = 0;
...
static int ldi_dir_reg_reg(vm_t *vm, prog_t *prog)
{
    short int sum[2] = {0};
    int reg[2] = {0};
...
{
    if (arg[1] == T_DIR)
        return ldi_dir_dir_reg(vm, prog);
    if (arg[1] == T_REG)
        return ldi_dir_reg_reg(vm, prog);
    modify_carry(prog, false);
    return 84;
}
