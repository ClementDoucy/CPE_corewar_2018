...
static int or_dir_reg_reg(vm_t *vm, prog_t *prog)
{
    int reg[2] = {0};
    int value_to_or = 0;
    ...
    my_memcpy(&value_to_or, &vm->mem[(prog->pc + 2) % MEM_SIZE], DIR_SIZE);
...
static int or_dir_dir_reg(vm_t *vm, prog_t *prog)
{
    int reg = 0;
    int value_to_or[2] = {0};

    my_memcpy(&value_to_or[0], &vm->mem[(prog->pc + 2) % MEM_SIZE], DIR_SIZE);
    my_memcpy(&value_to_or[1], &vm->mem[(prog->pc + 6) % MEM_SIZE], DIR_SIZE);
...
static int or_dir_ind_reg(vm_t *vm, prog_t *prog)
{
    int reg = 0;
    short int index = 0;
    int value_to_or[2] = {0};
...
{
    if (arg[1] == T_REG)
        return or_dir_reg_reg(vm, prog);
    if (arg[1] == T_DIR)
        return or_dir_dir_reg(vm, prog);
    if (arg[1] == T_IND)
        return or_dir_ind_reg(vm, prog);
    modify_carry(prog, false);
    return 84;
}
